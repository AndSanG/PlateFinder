...
- **Routers** — `@Observable @MainActor` types in the domain layer, injected via SwiftUI `.environment(...)`; `AppRouter` owns presentation flags such as `isHistoryPresented` (no `NavigationPath` — domain forbids SwiftUI import); `IntentRouter` bridges AppIntents to the SwiftUI tree
